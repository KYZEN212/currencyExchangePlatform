...
<body class="admin-theme dark-theme">
    ...
    <div id="unbanModal" class="modal">
        <div class="modal-content">
           ...
           <!-- Replace the existing unban form with this AJAX version -->
<form id="unbanForm" class="p-6">
    ...
    <div class="flex space-x-3">
        <button type="submit" class="flex-1 bg-green-600 hover:bg-green-700 text-white font-bold py-3 px-4 rounded-lg transition-colors">
            <span class="spinner hidden"><i class="fas fa-spinner fa-spin"></i> Processing...</span>
            <span class="button-text">Unban User</span>
        </button>
    ...
    form.reset(); // Reset any previous form state
    ...
    buttonText.textContent = 'Unban User';
...
document.getElementById('unbanForm').addEventListener('submit', function(e) {
    ...
    .then(html => {
        ...
        if (temp.querySelector('form[action="admin.php"]')) {
            window.location.href = 'admin.php';
            return;
        }
        
        // Find the message in the response
        const messageDiv = temp.querySelector('.bg-green-100, .bg-red-100');
        if (messageDiv) {
            // Show success/error message
            showNotification(messageDiv.textContent.trim(), 
                           messageDiv.classList.contains('bg-green-100') ? 'success' : 'error');
            
            // Close the modal and refresh the user list
            closeUnbanModal();
            loadUsers(); // Refresh the user list
        } else {
            // If we can't find a message, reload the page
...
// Add this function to refresh the user list
function loadUsers() {
    fetch('admin_user_management.php?embed=1')
        .then(response => response.text())
        .then(html => {
            const temp = document.createElement('div');
            temp.innerHTML = html;
            const tbody = temp.querySelector('tbody#usersTbody');
            if (tbody) {
                document.getElementById('usersTbody').innerHTML = tbody.innerHTML;
            }
        });
}
