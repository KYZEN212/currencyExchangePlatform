...
/* Remaining amount text (optional, you can keep or remove) */
.trade-remaining {
                    ...
                    <?php foreach ($trades as $trade): ?>
                        ...
                        <div class="trade-card <?php echo ($trade['trade_type'] === 'sell') ? 'sell' : 'buy'; ?>">
                            <div class="trade-header">
                                <div>
                                    <div class="trade-poster">
                                        Posted by: <strong><?php echo htmlspecialchars($trade['seller_username']); ?></strong>
...
<?php if ($show_remaining): ?>
    <?php
        // Fix for remaining amount display
        ...
        // Determine which currency to show and calculate correct amount
        if ($trade_type === 'buy') {
            // For BUY posts: show remaining in BUY currency
            $currency_symbol = $buy_symbol;
            
            // Check if this is a cross FX pair (USD/JPY, USD/THB, THB/JPY)
            $fx_set = ['USD','JPY','THB'];
            $is_cross_fx = in_array($buy_symbol, $fx_set) && in_array($sell_symbol, $fx_set) && $buy_symbol !== $sell_symbol;
            
            if ($is_cross_fx) {
                // For cross FX BUY posts, remaining is stored in BUY currency
                $display_amount = $remaining_raw;
            } else {
                // For other BUY posts (including MMK-FOREIGN)
                // Check if it's MMK-FOREIGN pair
                list($is_mmk_foreign, $foreign_symbol) = is_mmk_foreign_pair($buy_symbol, $sell_symbol);
                
                if ($is_mmk_foreign && $buy_symbol === 'MMK') {
                    // Buying MMK with FOREIGN: remaining is stored in MMK (BUY), but need to convert
                    // Actually, for BUY posts, remaining is stored in BUY currency (MMK)
                    $display_amount = $remaining_raw;
                } else {
                    // Default: remaining is in BUY currency
                    $display_amount = $remaining_raw;
                }
            }
        } else {
            // For SELL posts: show remaining in SELL currency
            $currency_symbol = $sell_symbol;
            
            // Check if this is MMK-FOREIGN SELL post
            list($is_mmk_foreign, $foreign_symbol) = is_mmk_foreign_pair($buy_symbol, $sell_symbol);
            
            if ($is_mmk_foreign) {
                // For MMK-FOREIGN SELL posts, remaining is stored in SELL currency
                $display_amount = $remaining_raw;
            } else {
                // For other SELL posts, remaining is stored in BUY currency, need to convert to SELL
                $fx_set = ['USD','JPY','THB'];
                $is_cross_fx = in_array($buy_symbol, $fx_set) && in_array($sell_symbol, $fx_set) && $buy_symbol !== $sell_symbol;
                
                if ($is_cross_fx) {
                    // For cross FX SELL posts, remaining is stored in BUY, convert to SELL
                    $display_amount = $remaining_raw * $exchange_rate;
                } else {
                    // For other pairs, use ratio
                    $amount_bought = (float)$trade['amount_bought'];
                    $amount_sold = (float)$trade['amount_sold'];
                    if ($amount_bought > 0) {
                        $ratio = $remaining_raw / $amount_bought;
                        $display_amount = $amount_sold * $ratio;
                    } else {
                        $display_amount = 0;
                    }
                }
        ...
        if ($currency_symbol === 'USD') {
            $remaining_formatted = number_format($display_amount, 2);
        } else {
            $remaining_formatted = number_format((int)round($display_amount), 0);
        }
    ?>
    <div class="trade-remaining">
        Only <?php echo $remaining_formatted . ' ' . $currency_symbol; ?> remaining
    </div>
<?php endif; ?>
                            </div>
