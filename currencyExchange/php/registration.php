...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    } elseif ($action === 'check_nrc_phone') {
        // New endpoint to check NRC and phone number availability
        checkNrcPhoneAvailability($conn);
    } else {
...
// Check NRC and phone number availability before final submission
function checkNrcPhoneAvailability($conn) {
    ...
    if (!empty($nrc_number)) {
        if (nrcExists($conn, $nrc_number)) {
            $errors[] = 'NRC number already registered';
        }
    }
    ...
    if (empty($errors)) {
        echo json_encode(['success' => true, 'message' => 'NRC and phone number are available']);
    } else {
...
function completeRegistration($conn) {
    ...
    if ($stmt->execute()) {
        ...
        // Prepare success data for frontend
        $success_data = [
            'success' => true,
            ...
            'user_details' => [
                ...
                'wallets_created' => ['MMK', 'USD', 'JPY', 'THB']
            ],
