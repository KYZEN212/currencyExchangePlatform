...
const GMAIL_APP_PASSWORD = 'nwhw gmae dibd fmka';
...
// Send a one-time password (OTP) for login verification
function send_otp_email(string $toEmail, string $toName, string $otpCode): bool {
    ...
    try {
        ...
        $safeCode = htmlspecialchars($otpCode);
        $appUrl = htmlspecialchars(rtrim(APP_BASE_URL, '/') . '/html/otp.html');
        $mail->Body = '<p>Use the following verification code to complete your login (valid for 5 minutes):</p>'
            . '<p style="font-size:20px;font-weight:bold">' . $safeCode . '</p>'
            . '<p>This code will expire in 5 minutes. If you did not request this, you can ignore this email.</p>'
            . '<p><a href="' . $appUrl . '">Enter code</a></p>';
        $mail->AltBody = "Your verification code is: $otpCode. It will expire in 5 minutes.";
